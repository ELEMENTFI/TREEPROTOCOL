...
contract MasterChef is Context,Ownable,Initializable {
    ...
    function pendingBlack(address _user) external view returns (uint256) {
        if(rewardStartDate <= block.timestamp ){
            ...
            if (block.number > pool.lastRewardBlock && totalStakedTokens != 0) {
                ...
                accBlackPerShare = accBlackPerShare.add(blackReward.mul(1e18).div(totalStakedTokens));
            }
            return user.amount.mul(accBlackPerShare).div(1e18).sub(user.rewardDebt);
        }
        else
            return 0;
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 blackReward = multiplier.mul(blackPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accBlackPerShare = pool.accBlackPerShare.add(blackReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBlackPerShare).div(1e18);
    ...
    function withdraw(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBlackPerShare).div(1e18);
        uint256 pending = currentUserBalance.mul(pool.accBlackPerShare).div(1e18).sub(CurrentRewardBalance);
        if(pending > 0) {
   ...
   function claimReward() external {
        ...
        uint256 CurrentRewardBalance= user.rewardDebt;
        user.rewardDebt = user.amount.mul(pool.accBlackPerShare).div(1e18);
        uint256 pending = user.amount.mul(pool.accBlackPerShare).div(1e18).sub(CurrentRewardBalance);
  ...
  //----------setter---------------------------------------------//
    ...
    function setTokenaddress(address  _token) public onlyOwner{
        /** address token = IBEP20(_token); */
        poolInfo.lpToken=IBEP20(_token);
    }
